...
export async function generateTypes(resolved: ResolvedConfig): Promise<GenerateTypesResult> {
  ...
  // Type-gen surfaces source strings to users (so `t('Sign out')` narrows
  // correctly). For plain strings the catalog value IS the source — use it.
  // Tree entries fall back to their hashed key, which is what `<T>` callers
  // see if they ever look up by key directly.
  const keys = Array.from(
    new Set(Object.entries(catalog).map(([k, v]) => (typeof v === 'string' ? v : k))),
  ).sort();
