...
/**
 * Emit a `.d.ts` augmenting `AutotranslateCatalog` with the catalog keys.
 * Targets both `@autotranslate/core` (canonical) and `@autotranslate/react`
 * (back-compat) so `t()` and `useT()` both narrow.
 */
export async function generateTypes(resolved: ResolvedConfig): Promise<GenerateTypesResult> {
  ...
  const augmentation = keys.map((k) => `    ${JSON.stringify(k)}: true;`).join('\n');
  const body =
    keys.length === 0
      ? `${banner}\nexport {};\n`
      : `${banner}
